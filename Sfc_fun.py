...
class Sfc_fun:
    ...
    def createVm(self):
        ...
        headers = {"Accept": "application/json" ,"Content-Type" : "application/json" , "X-Auth-Token": self.token_id}
        ...
        headers = {"Accept": "application/json","Content-Type" : "application/octet-stream" , "X-Auth-Token": self.token_id}
        ...
        for y in range(0 ,3):
            ...
            if (response.status_code == 202):
                ...
                #print data
                time.sleep(10)
                url = 'http://'+self.os_hostname+':8774/v2.1/'+self.tenant_id+'/servers/'+self.vm_id+'/os-interface'
                headers = {"Accept": "application/json" ,"Content-Type" : "application/json", "X-Auth-Token": self.token_id}
                response = requests.post(url, headers= headers, data = data)
                #print response.status_code
                if (response.status_code == 200):
    ...
    def checkFlowAdded(self):
        ...
        time.sleep(5)
        response = requests.get('http://'+self.onos_hostname+':8181/onos/v1/flows', auth=("karaf", "karaf"))
...
####################################################################
    ...
    def attachInterface(self):
        ...
        data = json.dumps(Dicdata,indent=4)
        #print data        
        url = 'http://'+self.os_hostname+':9696/'+self.osver+'/routers/'+self.router_id+'/add_router_interface'
        headers = {"Accept": "application/json" , "X-Auth-Token": self.token_id}
    ...
    def portChainDeviceMap(self):
        response = requests.get('http://'+self.onos_hostname+':8181/onos/vtn/portChainDeviceMap/'+self.PC_id, auth=("karaf", "karaf"))
    ...
    def portChainSfMap(self):
        response = requests.get('http://'+self.onos_hostname+':8181/onos/vtn/portChainSfMap/'+self.PC_id, auth=("karaf", "karaf"))
    ...
    def cleanup(self):
        ...
        if (response.status_code == 200):
            ...
            data = json.dumps(Dicdata1,indent=4)
            #print data
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/routers/'+self.router_id+'/remove_router_interface.json'
            headers = {"Accept": "application/json" , "X-Auth-Token": self.token_id}
            response = requests.put(url, headers= headers, data = data)
            if (response.status_code == 200):
                url = 'http://'+self.os_hostname+':9696/'+self.osver+'/routers/'+self.router_id
